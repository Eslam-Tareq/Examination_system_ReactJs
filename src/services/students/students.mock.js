...
const enrollments = students.map((s, idx) => {
    ...
    const status = grade >= course.Passing_Grade ? "Success" : "Fail";
    return {
        Stud_ID: s.Stud_ID,
        Course_ID: courseId,
        Used_Attempt: used,
...
export const buildStudentSummaries = (instructorId) => {
    const instructorCourses = mockCourses
        .filter((c) => c.Inst_ID === instructorId)
        .map((c) => c.Course_ID);
    const summaries = enrollments
