...
function buildMcq(quesId, examId, template, index) {
    return {
        quesId,
        examId,
...
function buildTf(quesId, examId, template, index) {
    return {
        quesId,
        examId,
        text: template.text + (index > 0 ? ` (${index + 1})` : ""),
...
function questionsForExam(examId, count, startQuesId) {
    const out = [];
    let qid = startQuesId;
    for (let i = 0; i < count; i++) {
        if (i % 2 === 0) {
            out.push(buildMcq(qid++, examId, MCQ_TEMPLATES[i % MCQ_TEMPLATES.length], i));
        }
        else {
            out.push(buildTf(qid++, examId, TF_TEMPLATES[i % TF_TEMPLATES.length], i));
        }
    }
    return out;
}
/** Exam 1–3: 20 questions each; 4–10: 10 each; 11–30: 5 each. */
function buildAllMockQuestions() {
    const all = [];
    let quesId = 1;
...
const sessionAdded = [];
const sessionDeletedIds = new Set();
const sessionUpdated = new Map();
let nextTempId = 100000;
function getMergedQuestionsForExam(examId) {
    const base = MOCK_QUESTIONS.filter((q) => q.examId === examId && !sessionDeletedIds.has(q.quesId));
    const updated = base.map((q) => sessionUpdated.get(q.quesId) ?? q);
    const added = sessionAdded.filter((q) => q.examId === examId);
    return [...updated, ...added];
}
export const getQuestionsMock = async (examId, page, pageSize) => {
...
export const updateQuestionMock = async (quesId, payload) => {
    await new Promise((r) => setTimeout(r, 300));
    const inAdded = sessionAdded.find((q) => q.quesId === quesId);
    if (inAdded) {
        if (payload.text != null)
            inAdded.text = payload.text;
        if (payload.type != null)
            inAdded.type = payload.type;
        if (payload.mark != null)
            inAdded.mark = payload.mark;
        if (payload.mcq != null)
            inAdded.mcq = {
...
export const deleteQuestionMock = async (quesId) => {
    await new Promise((r) => setTimeout(r, 200));
